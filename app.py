...
def login_dengan_api():
    ...
    if not st.session_state["api_terverifikasi"]:
        st.title("🌱 Login Portal Daily Mindfulness")
...
def generate_renungan_harian(api_key, tgl_lahir, gender, jam_lahir, kota_lahir):
    ...
    - Output harus murni berupa "Panduan Mindfulness" atau narasi perenungan diri yang elegan, logis, dan mudah dicerna.
...
if login_dengan_api():
    ...
    # HALAMAN UTAMA
    st.title("📖 Portal Daily Mindfulness")
    ...
    with col1:
        tgl_lahir = st.date_input("📅 Tanggal Lahir", min_value=datetime.date(1920, 1, 1), max_value=datetime.date.today(), format="DD/MM/YYYY")
    ...
    with col2:
        ...
        if tahu_jam:
            # Menggunakan text_input agar bisa format AM/PM (time_input bawaan streamlit formatnya 24 jam)
            jam_lahir_str = st.text_input("⏰ Jam Lahir (Gunakan AM/PM)", placeholder="Contoh: 08:30 AM atau 02:15 PM")
        else:
            jam_lahir_str = "Tidak diketahui"

    # Logika peringatan jika jam/kota kosong
    if not kota or not tahu_jam:
        st.info("💡 **Catatan Penting:** Hasil panduan akan jauh lebih spesifik, mendalam, dan presisi jika Anda melengkapi data **Kota** dan **Jam Kelahiran** Anda.")

    if st.button("✨ Hasilkan Panduan Hari Ini"):
        if tahu_jam and not jam_lahir_str:
             st.warning("Silakan masukkan jam lahir Anda atau hapus centang 'Saya tahu jam lahir saya'.")
        else:
            with st.spinner("Menyelaraskan data dan menyusun panduan personal Anda hari ini..."):
                
                # Format tanggal lahir untuk prompt
                tgl_kirim = tgl_lahir.strftime("%d %B %Y")
                ...
                if error:
                    ...
                    st.success(f"Panduan Mindfulness untuk Anda hari ini ({datetime.date.today().strftime('%d %B %Y')}):")
